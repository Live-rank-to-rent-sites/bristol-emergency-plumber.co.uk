...
function generateSitemapUrls(): SitemapUrl[] {
  ...
  for (const [i, location] of INDEXED_LOCATIONS.entries()) {
    // Main location page (Bristol gets higher priority)
    const isBristol = location.slug === 'bristol';
    urls.push({
      loc: `/locations/${location.slug}/`,
      priority: isBristol ? 0.9 : 0.8,
      changefreq: 'weekly',
      lastmod: getStaggeredDate(isBristol ? 1 : 2 + i),
    });
